...
class Contributions:
    ...
    def leaderboard(self, user_ids: List[str], start_date: datetime = None, end_date: datetime = None) -> List[UserStats]:
        stats = [self._user_score(user_id, start_date, end_date)
                 for user_id in user_ids]
        return sorted(stats, key=lambda stat: stat._score, reverse=True)

    def _user_score(self, user_id: str, start_date: datetime = None, end_date: datetime = None) -> UserStats:
